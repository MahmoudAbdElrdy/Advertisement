...
namespace Infrastructure {
  public class ImageService : IImageService {
    ...
    public ImageService(IHttpContextAccessor httpContext, IHostingEnvironment env, IOptions<ImageOption> imgOption, ILogger<ImageService> logger) {
      ...
      _imgOption = imgOption.Value;
        ...
        public async Task<Image> SaveImageAsync(Image image, byte[] dtoFile)
        {
            try
            {
                ...
                {
                    throw new ApiException(ApiExeptionType.InvalidImage, validationError);
                }
                ...
                else
                {
                    ...
                    CreateFolders(path);
                    image.Id = Guid.NewGuid().ToString();
                    var imageUrl = Path.Combine(path, image.Id + DateTime.Now.Ticks + "." + GetImageFormat(dtoFile));
                    image.Imageurl = image.Id + "." + GetImageFormat(dtoFile);
            ...
            {
                throw new ApiException(ApiExeptionType.FaildSaveImage, e.Message);
            }
        }
      ...
      throw new ApiException(ApiExeptionType.InvalidImage);
    }

        public  bool TryGetFromBase64String(string input, out byte[] output)
        {
            output = null;
            try
            {
    ...
    public void DeleteImageIfExist<T>(T id) {
      ...
      try {
        ...
        throw new ApiException(ApiExeptionType.FaildDeleteImage);
      }
    }
